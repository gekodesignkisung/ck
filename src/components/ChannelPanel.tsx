...
export default function ChannelPanel({
    ...
    for (const agent of agents) {
      ...
      try {
        ...
        const reply: Message = {
          ...
          content: data.content ?? data.error ?? '(응답 없음)',
          createdAt: new Date(),
        };
  ...
  return (
    ...
    >
      {/* Panel header */}
      <div className="flex h-[40px] items-center px-[15px] shrink-0 gap-2" style={{ background: 'rgba(51,51,51,0.05)' }}>
        <div className="flex items-center min-w-0 flex-1">
          <span className="text-[14px] font-semibold text-[#292929] whitespace-nowrap">
            {titlePrefix}{panel.title}
          </span>
        </div>
        {isChannel && (
          <div className="flex items-center gap-1 text-[14px] font-semibold text-[#666] whitespace-nowrap">
            <div className="flex gap-[4px] h-[24px] items-center justify-center px-[10px]">
              <span>파일</span>
              <span>{panel.fileCount ?? 0}</span>
            </div>
            <div className="flex gap-[4px] h-[24px] items-center justify-center px-[10px]">
              <span>멤버</span>
              <span>{panel.memberCount ?? 0}</span>
            </div>
          </div>
        )}
        <button
      ...
      {/* Input area */}
      <div className="flex flex-col gap-[6px] p-[15px] shrink-0">
